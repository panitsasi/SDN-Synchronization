...
class Trainer(object):
    ...
    def run_games_for_agents(self):
        ...
        for agent_number, agent_class in enumerate(self.agents):
            self.agent_name = agent_class.agent_name
        ...
        #         self.agent_rand_score_results = [results[5]  for results in  self.results[agent_name]]
        #         self.agent_robin_score_results = [results[6]  for results in  self.results[agent_name]]
        ...
        # plt.show()
        # plt.plot(self.downCount)
        # plt.plot(self.UpCount)
        # plt.plot(self.newCount)
        # plt.show()
        return self.results
    ...
    def run_games_for_agent(self, agent_number, agent_class):
        ...
        for run in range(self.config.runs_per_agent):
            ...
            agent_config.seed = 10
            print("RANDOM SEED " , agent_config.seed)
            game_scores, rolling_scores, time_taken, rand_scores, robin_scores, self.downCount, self.UpCount, self.newCount = agent.run_n_episodes()
            print("Time taken: {}".format(time_taken), flush=True)
            self.print_two_empty_lines()
            agent_results.append([game_scores, rolling_scores, len(rolling_scores), -1 * max(rolling_scores), time_taken, rand_scores, robin_scores, self.downCount, self.UpCount, self.newCount])
            ...
            data = {
                ...
                "algorithm":self.agent_name,
                "episodes":self.config.num_episodes_to_run,
                "learning_rate": agent_config.hyperparameters["learning_rate"],
                "batch_size": agent_config.hyperparameters["batch_size"],
                "buffer_size": agent_config.hyperparameters["buffer_size"],
                "epsilon": agent_config.hyperparameters["epsilon"],
                "epsilon_decay_rate_denominator": agent_config.hyperparameters["epsilon_decay_rate_denominator"],
                "discount_rate": agent_config.hyperparameters["discount_rate"],  
                "tau": agent_config.hyperparameters["tau"],
                "update_every_n_steps": agent_config.hyperparameters["update_every_n_steps"],
                "linear_hidden_units": agent_config.hyperparameters["linear_hidden_units"],   
                "learning_iterations": agent_config.hyperparameters["learning_iterations"],
            ...
            with open("hyperparameter_tuning.json", 'a+') as file:
                json.dump(data, file)
                file.write(',')
                file.write('\n')
    ...
    def visualise_overall_agent_results(self, agent_results, agent_name, show_mean_and_std_range=False, show_each_run=False,
        ...
        if show_mean_and_std_range:
            ...
            roundWrites = self.agent_rand_score_results[0]
            robinWrites = self.agent_robin_score_results[0]
            rows = zip(x_vals, roundWrites ,robinWrites )
            filename = 'randArobin.csv'
            with open(filename, 'w', newline="") as file:
                writer = csv.writer(file)
                writer.writerow(["Episode","RandScores","RobinScores"])
                for row in rows:
                    writer.writerow(row) 
            
            rows = zip(x_vals, mean_results)
            if self.config.NNrestarter == True:
                filename = 'NN_retraining.csv'
            else:
                filename = 'adaptive.csv'
            with open(filename, 'w', newline="") as file:
                writer = csv.writer(file)
                if self.config.NNrestarter == True:
                    writer.writerow(["Episode","RewardRetr"])
                else:
                     writer.writerow(["Episode","RewardAdapt"])
                for row in rows:
                    writer.writerow(row) 
                    
            '''        
            roundWritesNorm = [x / max(mean_results) for x in roundWrites] #roundWrites / max(roundWrites)
            robinWritesNorm = [x / max(mean_results) for x in robinWrites] #robinWrites / max(robinWrites)
            rows = zip(x_vals, roundWritesNorm ,robinWritesNorm )
            filename = 'randArobinNormal.csv'
            with open(filename, 'w', newline="") as file:
                writer = csv.writer(file)
                writer.writerow(["Episode","RandScores","RobinScores"])
                for row in rows:
            ...
            mean_resultsNorm =  [x / max(mean_results) for x in mean_results]# mean_results / max(mean_results)
            rows = zip(x_vals, mean_resultsNorm)
            if self.config.NNrestarter == True:
                filename = 'NN_retrainingNorm.csv'
            else:
                filename = 'adaptiveNorm.csv'
            with open(filename, 'w', newline="") as file:
                writer = csv.writer(file)
                if self.config.NNrestarter == True:
                    writer.writerow(["Episode","RewardRetr"])
            ...
            rows = zip(x_vals, self.newCount, self.UpCount, self.downCount)
            filename = 'disruptions.csv'
            with open(filename, 'w', newline="") as file:
                writer = csv.writer(file)
                writer.writerow(["Episode","NewUps","NetUps","NetDowns"])
                for row in rows:
                    writer.writerow(row)    
        ...
        if self.config.baselines == True:
            ax.plot(x_vals, self.agent_rand_score_results[0], color = 'r')
            ax.plot(x_vals, self.agent_robin_score_results[0], color = 'g')
