...
class DQN(Base_Agent):
    ...
    def __init__(self, config):
        ...
        self.effective_episode_number = 0
        self.config = config
    ...
    def step(self):
        ...
        while not self.done:
            ...
            if self.networkChange == True:
                self.action_size = int(self.get_action_size())
                self.state_size += 1
                stateVal = self.state_size
                self.memory = Replay_Buffer(self.hyperparameters["buffer_size"], self.hyperparameters["batch_size"], self.config.seed, self.device)
                self.q_network_local.addInputNodes([1]*(stateVal-1),[1]*stateVal,self.environment.budget)
                self.q_network_target.addInputNodes([1]*(stateVal-1),[1]*stateVal,self.environment.budget)
                self.q_network_optimizer = optim.Adam(self.q_network_local.parameters(),
                                              lr=self.hyperparameters["learning_rate"], eps=1e-4)
                
                abcd = 1
                self.effective_episode_number = int(self.config.hyperparameters["epsilon_decay_rate_denominator"] * 3)
                #self.effective_episode_number = 0
            ...
            if self.time_for_q_network_to_learn():
                for _ in range(self.hyperparameters["learning_iterations"]):
                    self.learn()
            if skip_experience == False:
                self.save_experience()
            self.state = self.next_state #this is to set the state for the next iteration
            self.global_step_number += 1
        self.episode_number += 1
        self.effective_episode_number += 1
    ...
    def pick_action(self, state=None):
        ...
        if self.config.qExcluder == True:
            action_list = self.environment.action_list
            for q in range(len(action_list)):
                action_vector = action_list[q]
                for entry in range(len(action_vector)):
                    if action_vector[entry] == 1 and state[0,entry] < 0:
                        action_values[0,q] = min(action_values[0]) - 100

                        
        action = self.exploration_strategy.perturb_action_for_exploration_purposes({"action_values": action_values,
                                                                                    "turn_off_exploration": self.turn_off_exploration,
                                                                                    "episode_number": self.effective_episode_number})
    ...
    def reset_neural_network(self):
        '''
        count = 0
        for name, param in self.q_network_local.named_parameters():
